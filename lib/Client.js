...
module.exports = std.Class(Connection, function(supr) {
    ...
    var LATEST_TIME = -1
    var EARLIEST_TIME = -1
    var MAGIC_UNCOMPRESSED = 0
    var MAGIC_COMPRESSED = 1
    ...
    var fetchEncodeMessageCompressed= function(codec) {
        if(codec == 1) {
            throw "Gzip not implemented"
        } else if(codec == 2 && !SNAPPY) {
            SNAPPY = require("snappy")
        }
        return function(message) {
            var magicAndCrcAndMessage = std.pack('CCN', MAGIC_COMPRESSED, codec, std.crc32(message)) + message
            return std.pack('N',magicAndCrcAndMessage.length) + magicAndCrcAndMessage
        }
    }
    ...
    this._init = function(opts) {
        ...
        if(opts.compressionCodec > 0) {
            ...
            // alternative: send_defaults.encode = this._encodeSendRequestCompressedInner.bind(this)
            // alternative: configurable parameter of n messages per compressed message.
            send_defaults.encode = this._encodeSendRequestCompressedOutter.bind(this)
        } else {
    ...
    this._encodeSendRequestUncompressed = function(t) {
        var encodedMessages = ''
        for (var i = 0; i < t.messages.length; i++) {
            encodedMessages += this._encodeMessageUncompressed(t.message[i])
        }
    ...
    this._encodeMessageUncompressed = function(message) {
        var magicAndCrcAndMessage = std.pack('CN', MAGIC_UNCOMPRESSED, std.crc32(message)) + message
        return std.pack('N',magicAndCrcAndMessage.length) + message
    }

    this._encodeSendRequestCompressedOutter = function(t) {
        var encodedMessages = ''
        for (var i = 0; i < t.messages.length; i++) {
            encodedMessages += this._encodeMessageUncompressed(t.message[i])
        }
        if(this._isCompressedTopic(t.topic)) {
            var compressedMessages = SNAPPY.compressSync(encodedMessages)
            encodedMessages = this._encodeMessageCompressed(compressedMessages)
        }
    ...
    this._encodeSendRequestCompressedInner = function(t) {
        if(!this._isCompressedTopic(t.topic))
            return this._encodeSendRequestUncompressed(t)

        var encodedMessages = ''
        for (var i = 0; i < t.messages.length; i++) {
            var message = SNAPPY.compressSync(this._encodeMessageUncompressed(t.message[i]))
            encodedMessage += this._encodeMessageCompressed(message)
        }
