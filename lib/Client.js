...
module.exports = std.Class(Connection, function(supr) {
    ...
    this._encodeSendRequestCompressed = function(t) {
        if(!this._isCompressedTopic(t.topic))
            return this._encodeSendRequestUncompressed(t)

        var encodedMessages = ''
        var batch = ''
        var remainingBatch = this._compressionBatchSize
        function appendBatch(){
            if(this._compressionCodec == 2) {
                batch = SNAPPY.compressSync(batch)
            }
            encodedMessages += this._encodeMessageCompressed(batch)

            batch = ''
            remainingBatch = this._compressionBatchSize
        }
        for (var i = 0; i < t.messages.length; ++i) {
            batch += this._encodeMessageUncompressed(t.message[i])
            if(--remainingBatch <= 0)
                appendBatch()
        }
        if(remainingBatch < this._compressionBatchSize)
            appendBatch()
